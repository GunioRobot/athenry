  ...
  class Setup
    ...
    def fetch
      announcing 'Fetching stage file' do
        if File.exists?("#{CONFIG.workdir}/#{CONFIG.stageurl}")
          ...
          cmd "wget -c #{CONFIG.stageurl} -O #{CONFIG.workdir}/#{CONFIG.stageurl}"
        end
      end
    ...
    def extract
      announcing 'Extracting stage file' do
          cmd "tar xvjpf #{CONFIG.workdir}/#{CONFIG.stageurl} -C #{CONFIG.workdir}/#{CONFIG.chrootdir}"
        end
    ...
    def snapshot
      announcing 'Fetching and extracting portage snapshot' do
        if File.exists?("#{CONFIG.workdir}/#{CONFIG.snapshoturl}")
          ...
          cmd "wget -c #{CONFIG.snapshoturl} -O #{CONFIG.workdir}/#{CONFIG.snapshoturl}"
        end
        cmd "tar xvjpf #{CONFIG.workdir}/#{CONFIG.snapshoturl} -C #{CONFIG.workdir}/#{CONFIG.chrootdir}/usr"
      end
